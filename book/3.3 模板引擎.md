...
通过 `app.set` 设置模板引擎为 ejs 和存放模板的目录。在 lottery 下新建 views 文件夹，在 views 下新建 users.ejs，添加如下代码：
...
通过调用 `res.render` 函数渲染 ejs 模板，res.render 第一个参数是模板的名字，这里是 users 则会匹配 views/users.ejs，第二个参数是传给模板的数据，这里传入 name，则在 ejs 模板中可使用 name。`res.render` 的作用就是将模板和数据结合生成 html，同时设置响应头中的 `Content-Type: text/html`，告诉浏览器我返回的是 html，不是纯文本，要按 html 展示。现在我们访问 `localhost:8888/users/haha`，如下图所示：
...
上一节：[3.2 路由](https://github.com/se7en-1992/lottery/blob/master/book/3.2%20%E8%B7%AF%E7%94%B1.md)

下一节：[3.4 Express 浅析](https://github.com/se7en-1992/lottery/blob/master/book/3.4%20Express%20%E6%B5%85%E6%9E%90.md)
