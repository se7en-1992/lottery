首先，我们新建一个目录 lottery，在该目录下运行 `npm init` 生成一个 package.json，如下所示：
...
以上代码的意思是：生成一个 express 实例 app，挂载了一个根路由控制器，然后监听 8888 端口并启动程序。运行 `node index`，打开浏览器访问 `localhost:8888` 时，页面应显示 hello, express。
